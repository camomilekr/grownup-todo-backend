...
| `parse-bigint.pipe.spec.ts` | 변환·2^53 초과 정밀도 보존·형식 어긋난 입력의 400 거절·자릿수 가드가 앞자리 0을 깨뜨리지 않는지 |
...
**`parseBigIntOrNull`은 `BigInt()`를 부르기 전에 자릿수부터 본다.** 값 비교(`> INT8_MAX`)와
결론이 같으므로 걸러지는 입력도 반환값도 달라지지 않는다 — 같은 판정을 더 이른 자리에서
낼 뿐이다. 그래도 두는 이유는 **비용의 상한이 이 코드 바깥에 있기 때문이다.** `BigInt(문자열)`
파싱은 자릿수가 길어질수록 급격히 비싸지는데(19자리 0.0001밀리초, 16,000자리 0.12밀리초,
1,000,000자리 38밀리초), 얼마나 긴 문자열이 여기까지 도달하는지는 Node HTTP 파서의 헤더
예산이 정한다. 기본값(`http.maxHeaderSize` 16384바이트)에서는 20,000자리가 핸들러에 닿기
전에 431로 거절되어 최악이 요청당 0.12밀리초대이지만, `--max-http-header-size`를 올려 둔
배포에서는 그만큼 비용이 따라 오른다. 가드는 그 상한을 바깥 설정에 맡기지 않고 함수 안으로
가져온다.

**그 가드는 문자열 길이가 아니라 앞자리 0을 뺀 유효 자릿수를 센다.** 앞자리 0은 값에 기여하지
않기 때문이고, 여기서 갈릴 수 있는 자리가 **둘**이라 spec이 각각을 입력으로 고정한다.

- 길이를 그대로 재면 `0`을 100개 붙인 `42`가 거절된다 → `0` × 100 + `42` → `42n`
- 전부 `0`이면 기여하는 자리가 없어 검색이 `-1`을 돌려준다. 이 경우를 따로 0으로 잡지 않고 그냥
  빼면 자릿수를 1 크게 세어 `0`이 열아홉 개인 입력이 상한을 넘는 것으로 오인된다 → `0` × 19 → `0n`

**둘째 단정은 자릿수가 19 이상이어야 제 일을 한다.** `'0'`이나 `'000'`으로 쓰면 자릿수를 1 크게
세도 상한 안이라 잘못된 구현에서도 통과해 아무것도 지키지 못한다. 실제로 리뷰에서 그 갈래를
지우는 변형이 전 스위트를 초록으로 통과시킨 적이 있다 — 짧은 입력으로 바꾸면 그 상태로 돌아간다.
