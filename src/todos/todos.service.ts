...
export class TodosService {
   ...
   *
   * **하나의 순간이 두 가지로 쓰인다.** 활성 판정에는 그대로(반열림:
   * `activeFrom <= at < activeUntil`, 사용자 최종 확정 — 서버가 시간 처리를 하지
   * 않는다), 붙일 기록을 찾는 데는 유저 타임존 기준 날짜 키로 바꿔서. 두 값이 다른
   * 순간에서 나오면 "활성인 목록"과 "그날의 기록"이 서로 다른 시점을 말하게
   * 되므로, 같은 `at`에서 만드는 것이 이 메서드의 일이다.
   *
  ...
  private assertRange(range: TodoHistoryRange): void {
    ...
    // 뒤집힘은 잘리기 전의 순간 기준으로 본다 — 유저 타임존을 읽기 전에 판정할 수 있다.
    // 잘린 날짜 기준 검사와 등가가 아니다 — 같은 날짜 안에서 뒤집힌 두 순간은 잘리면
    // 같은 날이 되어 통과한다. 순간 기준이 더 엄격하고, 그 거절을 단위 테스트가 고정한다.
