...
export class TodoTemplatesRepository {
   ...
   *
   * **활성 판정은 요청 순간과 활성 기간을 그대로 비교하는 반열림 구간이다** —
   * `activeFrom <= at < activeUntil`(사용자 최종 확정). 시작 순간은 포함되고 상한
   * 순간은 포함되지 않으며, 값이 비어 있으면 그쪽 제한이 없다는 뜻이다(시작이
   * 없으면 언제부터든, 상한이 없으면 기한 없이). 서버는 그 두 값에 날짜 의미를
   * 부여하지 않는다 — 시간 해석은 클라이언트의 몫이고, "그날 활성"이 아니라
   * "그 순간 활성"이다.
   *
   * 상한이 미포함인 이유는 순간에 "그날의 끝"이 없기 때문이다 — 포함으로 두면
   * "8월 31일까지"를 `23:59:59.999…` 같은 정밀도에 얽힌 값으로 표현해야 하고,
   * 미포함 상한이면 다음 날 자정 하나로 끝난다.
   *
   * **판정하는 값과 기록을 찾는 값이 다르다.** 활성은 순간(`at`)으로 판정하고, 붙여 줄
   * 기록은 유저 타임존 기준 날짜(`historiedOn`)로 찾는다 — 기록의 키가 날짜
   * 컬럼(`@db.Date`)이기 때문이다. 하나로 받아 여기서 변환하면 타임존을 이 계층이
   * 알아야 해서 판단이 계층을 넘는다. 같은 요청 순간에서 두 값을 만드는 것은
   * 부르는 쪽(Service의 `listDailyOn`)의 책임이다.
   *
    ...
    return this.prisma.todoTemplate.findMany({
      where: {
        ...
        AND: [
          ...
          {
            // 반열림의 미포함 상한이다. `gte`로 되돌리면 상한 순간이 포함으로
            // 바뀐다 — e2e의 "activeUntil과 같은 순간에는 빠지고" 테스트가 잡는다.
