...
- **대기 시간 0은 대기하지 않음이다.** 매니페스트가 `preStop: sleep`으로 대기를 대신하는 경우를 위해 남긴 값이다. **이 저장소의 매니페스트는 그 길을 택하지 않았다** — `preStop`을 두지 않고 `SHUTDOWN_DRAIN_DELAY_MS=8000`을 주입한다(`k8s/deployment.yaml`). 둘이 같은 창을 덮으므로 함께 두면 대기가 이중으로 쌓이고, 남길 쪽으로 이 국면을 택한 이유는 기다리는 동안 준비 확인을 503으로 바꿔 **알리면서** 기다리기 때문이다
...
`isShuttingDown()`은 readiness 프로브의 판정 근거다(`src/health/readiness.service.ts`). **liveness는 이 값을 보지 않는다** — 그 규칙의 근거와 실측은 `docs/k8s-local-verification.md` ① 절 한 곳에 있다.
...
- **자원별 타임아웃 10초** — k8s `terminationGracePeriodSeconds` 기본 30초를 드레인 대기·처리 중 요청 완료와 나눠 쓰도록 잡은 값이다(이 저장소 매니페스트의 예산은 드레인 8초 + 이 값 10초 + 여유 12초 = 30초. 계산은 `k8s/deployment.yaml`). 초과 시 warn 후 다음 자원으로 진행하고, 최종 방어선은 k8s의 SIGKILL이다. 건너뛴 해제 작업은 취소되지 않고 백그라운드에 남는다(JavaScript에 강제 취소가 없다) — 프로세스가 곧 종료되므로 실질적인 해는 없다
...
**드레인이 없애지 못하는 실패가 하나 남아 있다 — keep-alive 커넥션이다.** 이미 맺어진 커넥션은 드레인 동안 계속 이 파드로 요청을 보내고(정상 처리된다), HTTP 서버를 닫는 순간에 요청이 실려 있으면 그 하나가 실패한다 — 드레인은 닫는 시점을 미룰 뿐이다. 실측과 구별법은 `docs/k8s-local-verification.md` ④의 표에 있다. 없애려면 드레인 중 응답에 `Connection: close`를 실어 커넥션을 회수하거나 호출 측 재시도가 필요하고, **둘 다 아직 없다.**
