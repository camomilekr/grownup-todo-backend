...
| `worktree-drop.sh`가 "예상 밖의 변경이 남아 있다"로 멈춘다 | 조건은 둘이다 — ① `docs/plan`·`docs/dev-loop` 밖의 모든 커밋되지 않은 변경(`docs/`의 다른 새 문서도 여기 걸린다. 커밋을 빠뜨린 것일 수 있다) ② 그 두 폴더 안에서 **이미 커밋된** 문서의 수정·삭제. `--force`로 밀면 그 작업이 되돌릴 수 없이 사라지므로 스크립트는 목록만 보여 주고 멈춘다 — 확인해 옮기거나 지운 뒤 다시 실행한다. 이 경로에서는 아무것도 건드리지 않으므로(`node_modules`도 남는다) 재실행이 처음부터 그대로 진행된다. **`.gitignore`가 무시하는 파일은 이 검사에 오르지 않고** 워크트리와 함께 사라진다(워크트리의 `.env`가 그 경우다 — 루트 사본이라 의도된 결과다) |
| `worktree-drop.sh`가 "루트에 같은 경로의 문서가 있고 내용이 다르다"로 멈춘다 | `worktree-new.sh`가 루트 계획서를 워크트리로 복사하므로 같은 경로가 양쪽에 있는 것이 정상이고, 그 뒤 루트 쪽을 고쳤다면 꺼내기가 그 수정을 덮어쓴다. 스크립트는 복사 **전에** 목적지와 내용을 비교해 멈춘다 — 합칠지 판단한 뒤 다시 실행하거나, 워크트리 사본이 낡은 것이면 `--drop-docs`로 버린다 |
...
**`git add -N`의 흔적은 지문을 찍은 뒤에도 인덱스에 남는다.** 계획서와 개발 루프 기록이 intent-to-add로 등록된 상태가 되고, 그러면 **`git ls-files --others`에 더 이상 잡히지 않는다**(실측: git 2.50.1). 같은 파일이 `git status`에는 `A`로, `git diff HEAD`에는 `A`로 나오는데 **`git diff --cached HEAD`는 그 항목을 보고하지 않는다** — 미추적 파일을 찾는 코드를 쓸 때 한쪽만 물으면 지문을 찍은 워크트리에서 조용히 빈 목록을 받는다. 파일을 지워도 인덱스 항목이 삭제로 남아 `git worktree remove`가 거부하므로 `git reset -- <경로>`로 떼어 내야 한다(`worktree-drop.sh`가 그렇게 한다. 워크트리 인덱스는 `.git/worktrees/<이름>/index`라 저장소 루트에 영향이 없다).
